...
def attention(inputs, sequence_length):
    ...
    query_w = tf.Variable(
        tf.random_normal([sequence_length * hidden_size, hidden_size],
                         stddev=0.1))
    query_b = tf.Variable(tf.random_normal([hidden_size], stddev=0.1))
    query = tf.tanh(
        tf.tensordot(
            tf.reshape(inputs, [-1, sequence_length * hidden_size]),
            query_w,
            axes=1) + query_b)
...
def phase1_trainorload(model_name,
                       training,
                       test,
                       sequence_length,
                       batch_size,
            ...
            for epoch in range(num_epochs):
                ...
                if printlog and epoch % 10 == 0:
                    log.write("{} epoch: {}\t training loss: {:.6f}\n".format(
                        model_name, epoch, loss_train))

            saver.save(sess, "./{}/{}".format(MODEL_PATH, model_name))

        test_loss, alphas, y_hat, test_phase2_h = sess.run(
            [loss, alphas, y_hat, attention_output],
            feed_dict={
                X: test[:, 7 - sequence_length:7, :],
                Y: test[:, 7, 3],
                keep_prob_ph: 1.0
            })

        training_phase2_h = sess.run(
            attention_output,
            feed_dict={
                X: training[:, 7 - sequence_length:7, :],
                Y: training[:, 7, 3],
                keep_prob_ph: 1.0
            })
